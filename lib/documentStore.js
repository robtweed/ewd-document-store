...
var DocumentNode = function(documentStore, documentName, path) {
  ...
  this.key = documentName + ';' + path.join();
...
proto.lock = require('./proto/lock');
proto.unlock = require('./proto/unlock');

proto._set = require('./proto/set');
proto.enable_kvs = require('./proto/enable_kvs');
proto.enable_list = require('./proto/enable_list');
proto.enable_dom = require('./proto/enable_dom');
proto.enable_rdb = require('./proto/enable_rdb');
...
var DocumentStore = function(db) {
  ...
  var dn = this.DocumentNode;
  this.dbx_reg = {};

  this.use = function(documentName, ...subscripts) {
    if (subscripts.length === 1 && Array.isArray(subscripts[0])) {
      subscripts = subscripts[0];
    }
    return new dn(documentName, subscripts);
  };

  this.fn = function(functionName, ...args) {
    if (this.db.dbx) {
      let log = false;
      ...
      let temp = new dn('qewdTemp', process.pid);
      temp.$('function').value = functionName;
      temp.$('args').setDocument(args);
      if (log) temp.$('log').value = true;
      let result = this.db.dbx.function('fn^qewdInterface');
      if (!temp.$('log').exists) temp.delete();
      return result;
    }
  };

  this.sql = require('./proto/rdb/sql');
